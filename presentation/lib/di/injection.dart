...
void presentationInjection(DIContainer container) {
  container.registerLazySingleton<LoginBloc>(
      () => LoginBloc(InitLoginState(), container.get<AuthUseCase>(), container.get<NavigationService>()));
  ...
  container.registerLazySingleton<RegisterBloc>(() => RegisterBloc(
      InitRegisterState(), container.get<AuthUseCase>(), container.get<NavigationService>(), container.get<LoginBloc>()));

  container.registerLazySingleton<PostsListBloc>(() => PostsListBloc(InitPostsListState(),
      container.get<PostRepository>(), container.get<LoginBloc>(), container.get<NavigationService>()));

  container.registerLazySingleton<PostsWriteBloc>(() =>
      PostsWriteBloc(InitPostsWriteState.init(), container.get<PostRepository>(), container.get<NavigationService>()));

  container.registerLazySingleton<PostsReadBloc>(() => PostsReadBloc(InitPostsReadState(),
      container.get<PostRepository>(), container.get<NavigationService>(), container.get<LoginBloc>()));
